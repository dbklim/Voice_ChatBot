...
def tts(text, mode, filename_audio = None):
    ...
    command_line += "| sox -t wav - -r 32000 -c 1 -b 16 -t wav - >'" + os.path.dirname(sys.argv[0]) + filename_audio + "'"
    if mode == 'playback':
        subprocess.call(command_line, shell=True)
        sound = AudioSegment.from_wav(os.path.dirname(sys.argv[0]) + filename_audio)
        play(sound)
        # command_line += "| aplay" - не используется, т.к. при каждом обращении выводится сообщение от самого RHVoice в терминал
    elif mode == 'into_file':
        subprocess.call(command_line, shell=True)
    else:
        print('[E] Неподдерживаемый режим работы, проверьте правильность передаваемых параметров.')
        return
...
def main():
    tts('я вас не понимаю, извините', 'into_file', '/data/answer.wav')
