# Для поддержки TensorFlow-GPU (подробнее в https://github.com/Desklop/Docker_image_CUDA10.0_cuDNN7.5)
...
RUN /bin/bash -c "if [[ ! $(ls -A 'data') || ! -f 'install_files/RHVoice.zip' ]]; then ./install_files/download_train_data_and_models.sh && \
